-- SETUP ACTUAL HIERARCHY WITH REAL USER IDS
-- This script sets up the hierarchy using the actual user IDs from the system
...
-- 2. Set up the hierarchy structure
-- Daniel (Admin) -> Marcel (Führungskraft) -> (future subordinates)
-- Daniel (Admin) -> Philipp (Führungskraft) -> (future subordinates)
-- Daniel (Admin) -> Kosta (Führungskraft) -> (future subordinates)
-- Robin Ache (Führungskraft) -> (future subordinates)

-- Set Marcel as subordinate of Daniel
UPDATE users 
...
-- Test Philipp's subordinates (should be empty for now)
