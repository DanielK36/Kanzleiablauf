-- FINAL CLEANUP - Only essential fixes for current database
-- This script only fixes the essential issues without recreating tables

-- 1. Check current state
SELECT 'CURRENT STATE - User roles:' as status;
SELECT DISTINCT role FROM users;

-- 2. Update role constraint to correct structure
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check;
ALTER TABLE users ADD CONSTRAINT users_role_check 
CHECK (role IN ('admin', 'führungskraft', 'berater', 'trainee'));

-- 3. Map existing roles to correct structure
UPDATE users SET role = 'führungskraft' WHERE role IN ('top_leader', 'sub_leader');
UPDATE users SET role = 'berater' WHERE role = 'advisor';

-- 4. Add team leader columns if they don't exist
...
-- 6. Create function to get team analytics
...
-- 7. Create function for team performance data
...
SELECT 'FINAL STATE - Users with roles:' as status;
SELECT id, email, firstname, lastname, name, role, team_name FROM users ORDER BY role, name;

-- Success message
SELECT 'Essential fixes applied successfully!' as status;
